// com.antelope.ci.bus.server.portal.TelnetIO.java
/**
...
public class IO {
	...
	public void setCloseOut() {
		this.m_Out = new NoCloseDataOutputStream(m_Out);
	}

	public void setCloseIn() {
		this.m_In = new NoCloseDataInputStream(m_In);
	}

	public int skip() throws IOException {
		int n = m_In.available();
		if (n > 0) {
			byte[] b = new byte[n];
			int length = m_In.read(b);
			System.out.println("Skip " + length + " bytes: "
					+ new String(b, 0, length) + " | "
					+ StringUtil.bytes2hex(b));
			return length;
		}
		return 0;
	}
	...
	/**
	 * read input byte with ssh protocol. As '13' in method of 'read()' can not
	 * been catched, it must add this method for key in with ssh.
	 * 
	 * @return
	 * @throws IOException
	 */
	public int readSsh() throws IOException {
		int c = rawread();
		// using 'Enter' intead fo 'LR'.
		// Byte code of 'Enter' is 10.
		// Byte code of 'LR' is 13.
		if (c == 13) {
			return 10;
		}
		// if (c == 255) {
					...
					case NE_VAR_DEFINED:
						...
						case NE_VAR_OK:
							...
							log.debug("readNEVariables()::NE_VAR_OK:VAR=" + str
									+ " VAL=" + sbuf.toString());
							IO.this.m_ConnectionData.getEnvironment()

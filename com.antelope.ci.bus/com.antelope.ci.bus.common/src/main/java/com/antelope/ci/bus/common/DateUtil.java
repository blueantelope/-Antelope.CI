...
public class DateUtil {
	...
	private static int differTime(Date d1, Date d2, String timePattern,
			long differ) throws CIBusException {
		SimpleDateFormat timeFormat = new SimpleDateFormat(timePattern);
		String s1 = timeFormat.format(d1);
		String s2 = timeFormat.format(d2);
		try {
			return (int) ((timeFormat.parse(s1).getTime() - timeFormat
					.parse(s2).getTime()) / differ);
		} catch (ParseException e) {
			throw new CIBusException("", e);
		}
	}
	...
	public static boolean isDay(String date) {
		...
		return isDate(date, regex);
	}
	
	public static boolean isMinute(String date) {
		String regex = "^((19|20)\\d\\d)-(0?[1-9]|1[012])-(0?[1-9]|[12][0-9]|3[01]) ([0-2][0-9]):([0-5][0-9]):([0-5][0-9])$";
		return isDate(date, regex);
	}

	private static boolean isDate(String date, String regex) {

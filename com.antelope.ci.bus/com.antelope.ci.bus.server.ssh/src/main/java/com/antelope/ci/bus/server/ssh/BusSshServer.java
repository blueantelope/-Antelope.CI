...
import com.antelope.ci.bus.server.ssh.shell.BusSshShellProxyLauncher;
...
public abstract class BusSshServer extends BusServer {
	...
	private void configShell() throws CIBusException {
		BusSshShellLauncher shellLauncher = null;
		switch (condition.getLauncherType()) {
			case PROXY:
				shellLauncher = new BusSshShellProxyLauncher();
				break;
			case CONTAINER:
				shellLauncher = new BusSshShellContainerLauncher();
				break;
			default:
				break;
		}
		BusSshShellFactory shellFactory;
		if (shellLauncher == null) {
			if (condition.getLauncher_class() != null) {
				shellFactory = new BusSshShellFactory(condition.getLauncher_class());
			} else if (condition.getLauncher_className() != null
					&& condition.getLauncher_className().length() > 0) {
				shellFactory = new BusSshShellFactory(condition.getLauncher_className());
			} else {
				throw new CIBusException("", "create shell factory error");
			}
		} else {
			shellLauncher.setCondition(condition);
			shellFactory = new BusSshShellFactory(shellLauncher);
		}
